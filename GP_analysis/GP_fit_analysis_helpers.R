# Helper functions for the analysis of the GP after the sequential design.
...
get_updated_normalized_test_data = function(normalized_curr_data, 
                                            loc_of_new_data,
                                            percentile = NULL){
  # See get_normalized_full_data for further details.  The 0-1 scale may need
  # to be updated depending on the new values.
  output_files = list.dirs(paste(loc_of_new_data, "/compiled_data/", sep=""))
  
  # Start by collecting SPECIFICALLY the new data values:
  dfn_HF_newdata = NULL
  
  
  for(ijob in 1:length(output_files)){
    exp_num       = ijob-1
    data_location = paste(loc_of_new_data, '/compiled_data/output_', exp_num, '/', sep = "")
    
    file_name       = paste(data_location, 'partime', ijob-1, '.dat', sep = "")
    if(file.exists(file_name)){
      ...
      temp_row         = data.frame(sim_num            = rep(ijob-1, times = nrow(HF_flowdata)), 
  ...
  if(is.null(dfn_HF_newdata)){
    stop("no new data were created.")
  }
  
  # I need to see if the new data values require a re-normalization.
  # This is somewhat dirty, but I'm dropping a low-outlier experiment.  I could refit
  # the final GP, but this wouldn't solve the problem of every GP before it also being
  # in the wrong scale.  Hopefully now things will be in the proper range -- I may need
  # do design a different experiment depending on how frequently this happens.
  dfn_HF_newdata = dfn_HF_newdata[which(dfn_HF_newdata$sim_num!=51),]
  
  min_BT      = min(dfn_HF_newdata$total_travel_time)
  max_BT      = max(dfn_HF_newdata$total_travel_time)
  dfn_HF_data = NULL
  if( (normalized_curr_data$new_max < max_BT) | 
      (normalized_curr_data$new_min > min_BT) ){
    stop("The new test data are outside the bounds of the final GP 0-1 scale.  Consider refitting that GP.")
  } else {
    min_BT                           = normalized_curr_data$new_min
    max_BT                           = normalized_curr_data$new_max
    dfn_HF_newdata$total_travel_time = (dfn_HF_newdata$total_travel_time - min_BT) / (max_BT - min_BT) 
    dfn_HF_data                      = dfn_HF_newdata
  }
  ...
  for(sim_num in unique(dfn_HF_data$sim_num)){
    ...
    # Now grab the inputs:
    data_location = paste(loc_of_new_data, '/compiled_data/output_', sim_num, '/', sep = "")
    file_name     = paste(data_location, 'SA_params_', sim_num, '.dat', sep = "")
    param_data    = read.csv(file_name)
    
    temp_row      = data.frame(alpha_semi = round((param_data[1,3] - min_alpha_semicorr) /
                                                    (max_alpha_semicorr - min_alpha_semicorr), digits = 4),
                               beta_semi  = round((param_data[1,4]- min_beta_semicorr) /
                                                    (max_beta_semicorr - min_beta_semicorr), digits = 4),
                               sigma_semi = round((param_data[1,5]- min_sigma_semicorr) /
                                                    (max_sigma_semicorr - min_sigma_semicorr), digits = 4),
                               alpha_TPL  = round((param_data[1,1] - min_alpha_TPL_exp) /
                                                    (max_alpha_TPL_exp - min_alpha_TPL_exp), digits = 4),
                               p32        = round((param_data[1,2] - min_p32) /
                                                    (max_p32 - min_p32), digits = 4 ),
                               sim_num    = sim_num)
...
get_updated_normalized_full_data = function(normalized_curr_data, 
                                            ...
                                            data_loc, percentile = NULL){
  # This is an updated version of the same method from the sequential_design directory
  # that doesn't assume that we're in the same directory as the rest of the code.
  ...
  print(paste("finished adding in data for:", num_of_experiments_finished))
  ...
  if( (normalized_curr_data$new_max < max_BT) | 
      (normalized_curr_data$new_min > min_BT) ){
    # Then the normalization must be updated.
    updated_normalization               = TRUE
    prev_raw_sim_data                   = normalized_curr_data$raw_sim_data
    prev_range                          = (normalized_curr_data$new_max - 
                                             normalized_curr_data$new_min)
    prev_raw_sim_data$total_travel_time = prev_raw_sim_data$total_travel_time * prev_range +
      normalized_curr_data$new_min
    
    dfn_HF_data                         = rbind(dfn_HF_data, prev_raw_sim_data)
    
    # From this point, we essentially perform identical steps to the last function.
    min_BT                        = min(dfn_HF_data$total_travel_time)
    max_BT                        = max(dfn_HF_data$total_travel_time)
    dfn_HF_data$total_travel_time = (dfn_HF_data$total_travel_time - min_BT) / (max_BT - min_BT) 
  } else {
    # Don't update the normalization, just tack on the new data.
...
get_pdf_data                       = function(normalized_curr_data){
  ...
  # dfn_HF_data        = dfn_HF_data[ which(dfn_HF_data$total_travel_time <= quantile(dfn_HF_data$total_travel_time, 0.95) ),]
  ...
  for(sim_num in unique(dfn_HF_data$sim_num)){
    ...
    sim_response  = c(sim_response, pdf_feats$y)
    sim_x         = c(sim_x, pdf_feats$x)
    sim_numbers   = c(sim_numbers, rep(sim_num, times = length(pdf_feats$y)))
  }
  
  return(data.frame(x = sim_x, density = sim_response, sim_num = sim_numbers))
}

normalize_the_full_data_woOutliers = function(num_of_replications, num_of_experiments,
                                                  percentile = NULL){
  # This takes in the method assuming that the breakthrough times have been processed
  # somewhat.  I need to then recalculate the 'response' (the 10th percentile of
  # the PDF).
  ...
  for(sim_num in unique(dfn_HF_data$sim_num)){
    ...
    if(is.null(percentile)){
      ...
      # quant         = cdf_to_quant(cdf, pdf_feats$x)
      # pdf_feats     = quant(percentile)
      pdf_feats     = quantile(temp_data$total_travel_time, probs = percentile)
      sim_response  = c(sim_response, pdf_feats)
    }
...
get_updated_normalized_full_data_woOutliers = function(normalized_curr_data, 
                                            num_of_experiments_finished,
                                            num_of_new_replicates,
                                            data_loc, percentile = NULL){
  # This is an updated version of the same method from the sequential_design directory
  ...
  print(paste("finished adding from experiment:", num_of_experiments_finished))
...
get_updated_normalized_test_data_woOutliers = function(normalized_curr_data, 
                                            loc_of_new_data,
                                            percentile = NULL){
  # See get_normalized_full_data for further details.  The 0-1 scale may need
  # to be updated depending on the new values.
  ...
  for(ijob in 1:length(output_files)){
    ...
    if(file.exists(file_name)){
      ...
      # here is where i need to update the raw travel time data.
      # temp_row         = temp_row[which(temp_row$total_travel_time <= quantile(temp_row$total_travel_time, 0.95) ),]
      
      dfn_HF_newdata   = rbind(dfn_HF_newdata, temp_row)
    }
    print(paste("finished importing data from rep:", ijob))
  }
...
get_normalized_full_data_woOutliers = function(num_of_replications, num_of_experiments,
                                                data_loc,
                                                percentile = NULL){
