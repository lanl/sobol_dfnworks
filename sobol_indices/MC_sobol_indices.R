...
# TODO: Talk to Justin and Kelly about using the same design points to estimate
#       both the Sobol for the mean GP and for the dispersion GP.

estimate_sobol_indices = function(gp_model, num_samples, input_dimension, var_names = NULL){
  ...
  for(i in 1:input_dimension){
    for(j in 1:num_samples){
      ...
      first_order[[i]]  = first_order[[i]] + (1/num_samples) *
                           f_B_vector[j] * (f_A_sub_B_of_i[[i]][j] - f_A_vector[j])
      total_effect[[i]] = total_effect[[i]] + 0.5 * (1/num_samples) * 
